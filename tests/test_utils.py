...
from constants import DELIVERY_ADDITIONAL_TIME, ERRAND_TYPES, ErrandType

class TestUtils(unittest.TestCase):
    def test_create_city_grid(self) -> None:
        grid: np.ndarray = create_city_grid()
    ...
    def test_is_valid_road_location(self) -> None:
        # Test road locations
    ...
    def test_get_nearest_road_point(self) -> None:
    ...
    def test_calculate_travel_time(self) -> None:
        # Test same location
    ...
    def test_calculate_errand_time(self) -> None:
        start_location: Tuple[int, int] = (0, 0)
        end_location: Tuple[int, int] = (10, 10)
        
        for errand_type, base_time, _, _ in ERRAND_TYPES:
            errand: Errand = Errand(0, errand_type, base_time, 1.0, None)
            errand_time: int = calculate_errand_time(errand, start_location, end_location)
            
            expected_time: int = base_time + 20  # 20 is the Manhattan distance between (0,0) and (10,10)
            ...
            self.assertEqual(errand_time, expected_time, f"Failed for errand type: {errand_type.name}")
