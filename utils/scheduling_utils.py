...
from utils.time_utils import is_time_within_range

logger = logging.getLogger(__name__)

class SchedulingUtilities:
    @staticmethod
    def _is_within_working_hours(start_time: pd.Timestamp, end_time: pd.Timestamp) -> bool:
    ...
    def calculate_next_available_time(contractor: Contractor, customer: Customer, current_datetime: pd.Timestamp) -> Optional[pd.Timestamp]:
        ...
        while current_datetime <= schedule.index.get_level_values('Date').max():
            day_end = pd.Timestamp.combine(current_datetime.date(), WORK_END_TIME_OBJ.time())
            ...
            while current_datetime <= day_end:
                ...
                if len(available_slots) >= total_blocks and available_slots['Client_ID'].isnull().iloc[:total_blocks].all():
                    potential_end_time = current_datetime + total_time

                    if SchedulingUtilities._is_within_working_hours(current_datetime, potential_end_time):
    ...
    def is_valid_assignment(contractor: Contractor, customer: Customer, travel_start_time: pd.Timestamp, task_end_time: pd.Timestamp) -> bool:
        ...
        if not SchedulingUtilities._is_within_working_hours(travel_start_time, task_end_time):
            return False
