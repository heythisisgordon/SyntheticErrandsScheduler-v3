...
class ApplicationController:
    ...
    def _bind_events(self):
        self.event_manager.bind(EventManager.PROBLEM_DEFINED, self._on_problem_defined)
        self.event_manager.bind(EventManager.PROBLEM_GENERATED, self._on_problem_generated)
        self.event_manager.bind(EventManager.SOLUTION_GENERATED, self._on_solution_generated)
    ...
    def _on_problem_generated(self, event):
    ...
    def _on_generate_greedy_solution(self, event):
    ...
    def _on_solution_generated(self, event):
        schedule = event.schedule if hasattr(event, 'schedule') else None
        if schedule:
            self._display_contractor_schedule(schedule)
        self.ui_manager.enable_tab(self.main_frame.contractor_schedule_tab)

    def _display_contractor_schedule(self, schedule):
